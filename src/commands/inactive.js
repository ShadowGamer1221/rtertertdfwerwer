...
const config = {
    description: 'Sets a user inactive in the Roblox group.',
    aliases: [],
...
module.exports = {
    ...
    run: async (client, message, args) => {
        ...
        if(rankInGroup === client.config.firedRank) {
            embed.setDescription(`<:Bruh:862667091801800705> That user has already inactive.`);
            embed.setColor(client.config.colors.error);
            embed.setAuthor(message.author.tag, message.author.displayAvatarURL());
            embed.setTimestamp();
            return message.lineReply(embed);
        ...
        if(client.config.verificationChecks === true) {
            ...
            if(rankInGroup >= linkedUserRankInGroup) {
                embed.setDescription('<:Bruh:862667091801800705> You can only rank people with a rank lower than yours, to a rank that is also lower than yours.');
                embed.setColor(client.config.colors.error);
                embed.setAuthor(message.author.tag, message.author.displayAvatarURL());
                embed.setTimestamp();
                return message.lineReply(embed);
        ...
        embed.setDescription(`<a:fired:847902697340010517> **Success!** ${username} is now inactive.`);
