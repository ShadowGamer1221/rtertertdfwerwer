...
module.exports = {
    ...
    run: async (client, message, args) => {
        ...
        if(!mention) {embed.setDescription("You did not provide anyone to DM!"); return message.lineReply(embed)}
        mention = message.guild.member(mention)
        if(!mention) {embed.setDescription("I cannot DM this person as they are not in the server!"); return message.lineReply(embed)}
        const toSend = args.slice(1).join(" ")
        if(!toSend) {embed.setDescription("You did not provide what to send!"); return message.lineReply(embed)}
        let success = true
        try {
