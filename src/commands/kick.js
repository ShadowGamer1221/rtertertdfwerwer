...
module.exports = {
    ...
    run: async (client, message, args) => {
        ...
        if(!message.member.hasPermission("KICK_MEMBERS")) {embed.setDescription("You need the `KICK_MEMBERS` permission to use this command!"); return message.lineReply(embed)}
        if(!message.guild.me.hasPermission("KICK_MEMBERS")) {embed.setDescription("I do not have permission to kick people! Please give me the `KICK_MEMBERS` permission!"); return message.lineReply(embed)}
        ...
        if(!mention) {embed.setDescription("You did not provide anyone to kick!"); return message.lineReply(embed)}
        mention = message.guild.member(mention)
        if(!mention) {embed.setDescription("I cannot kick this person as they are not in the server!"); return message.lineReply(embed)}
        if(mention.id === message.author.id) {embed.setDescription("You cannot kick yourself!"); return message.lineReply(embed)}
        if(mention.roles && !message.member.roles.highest.position > mention.roles.highest.position) {embed.setDescription("Your highest role is too low to kick this member!"); return message.lineReply(embed)}
        if(mention.roles && !mention.kickable) {embed.setDescription("I cannot kick this member! Please insure my role is higher than who you mentioned!"); return message.lineReply(embed)}
        let good = true
